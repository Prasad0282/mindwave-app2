...
export const Chat: React.FC = () => {
  ...
  const animateText = async (messageId: string, text: string) => {
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      await new Promise((resolve) => setTimeout(resolve, 10)); // 0.01 second delay per word
    }
  ...
  return (
    <div className="h-screen flex bg-gray-900 overflow-hidden">
      ...
      <aside
        className={`
          w-64 bg-gray-800 flex-shrink-0 transition-all duration-300
      ...
      <main
        ...
        ${!isSidebarOpen && !isRightSidebarOpen ? "ml-0 mr-0" : ""}
        ${!isSidebarOpen ? "md:ml-0" : "md:ml-64"}
        ${!isRightSidebarOpen ? "md:mr-0" : "md:mr-64"}
      `}
      >
        {/* Header */}
        <header className="bg-gray-800 p-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-white text-xl font-semibold">MindWave Chat</h1>
          </div>
          <Button
            variant="ghost"
      ...
      <aside
        className={`
          w-64 bg-gray-800 flex-shrink-0 transition-all duration-300
          ${isRightSidebarOpen ? "translate-x-0" : "translate-x-64"}
          fixed md:relative right-0 h-full z-20
        `}
      >
        <div className="h-full flex flex-col p-4">
          <div className="flex items-center justify-between mb-6">
      ...
      {showFeedbackSuccess && (
        <div className="fixed bottom-4 right-4 bg-green-500 text-white px-4 py-3 rounded-lg flex items-center gap-2 animate-fade-up">
